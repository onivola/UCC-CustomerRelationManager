                    ...
                    <div id="sidebar-scroll">
                        ...
                        <div class="sidebar-content">
                            ...
                            <ul class="sidebar-nav">
                                ...
                                <li>
                                    <a href="/lists-devis-per-agent/{{ \Illuminate\Support\Facades\Auth::user()->name }}" class="active"><i class="gi gi-table sidebar-nav-icon"></i><span class="sidebar-nav-mini-hide">Devis</span></a>
                                </li>
                                <li>
                    ...
                    <div id="page-content">
                        ...
                        <div class="content-header">
                            <div class="header-section">
                                <h1>
                                    Listes des devis<br><small>Voici les listes de tous les devis que vous avez enregistré</small>
                                </h1>
                            </div>
                        </div>
                        <ul class="breadcrumb breadcrumb-top">
                            <li></li>
                            <li><a href="">Listes Devis</a></li>
                        </ul>
                        <!-- END Validation Header -->
                        <div class="block full">
                            <div class="block-title">
                                <h2><strong>Listes</strong> Devis</h2>
                            </div>
                                ...
                                <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($leads as $lead)
                                            @php
                                                $etatSignature = $lead->etat_signature;
                                            @endphp

                                            @if ($etatSignature != "aucun_aucun" && $etatSignature != null)
                                                <tr>
                                                    <td class="text-center">{{ \Carbon\Carbon::parse($lead->_crm_models_updated_at)->format('Y-m-d') }}</td>
                                                    <td class="text-center">{{ $lead->Noms_commerciaux }}</td>
                                                    <td><a href="javascript:void(0)">{{ $lead->name }} {{ $lead->first_name }}</a></td>
                                                    <td><a href="javascript:void(0)">{{ $lead->Adresse_postale }}</a></td>
                                                    <td class="text-center">{{ $lead->Numero_SIRET }}</td>
                                                    <td>{{ $lead->email }}</td>
                                                    <td class="text-center">{{ $lead->PR30WMCEE + $lead->PR50WMCEE + $lead->PR100WMCEE + $lead->HUB1600RWBCEE }}</td>
                                                    <td>
                                                        <span class="label label-success">Signé</span>
                                                    </td>
                                                    <td class="text-center">
                                                        <div class="btn-group">
                                                            <a href="/see-quote-before-sending-it/{{ $lead->crm_id }}" data-toggle="tooltip" title="Voir" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a>
                                                            <a href="/update-lead-list-to-devis/{{ $lead->crm_id }}" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                                                            <a href="/delete-lead/{{ $lead->crm_id }}" data-toggle="tooltip" title="Delete" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
                                                        </div>

                                                        <!-- Modal Modification contrat -->
                                                        <div class="modal fade" id="modal-contrat-delete-{{ $lead->crm_id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <form id="form-validation" action="/supprimer-contrat/{{ $lead->crm_id }}" method="post" class="form-horizontal form-bordered">
                                                                            <div style="margin: 22px !important;">Voulez-vous vraiment supprimer le contrat <strong>ID_Contrat</strong></div>
                                                                            <button style="margin-bottom: 22px !important;" type="submit" class="btn btn-warning dv-edit">SUPPRIMER</button>
                                                                        </form>
                                                                    </div>
                                                        ...
                                                        <!-- Fin du modal Modification contrat -->
                                                    </td>
                                                </tr>
                                            @endif
